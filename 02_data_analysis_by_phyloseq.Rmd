...
La colonne 1 correspond aux estimations d'abondances, et la 2 aux abondances observées. On effectue donc ici des estimations d'abondance. 
```{r}
...
Ici, nous avons effectué une PCoA avec l'indice de dissimilarité de Bray-Curtis. Il permet d'évaluer la distance, soit la dissimilarité entre deux échantillons.
```{r}
...
```
On observe une grande différence entre la distribution des données de mars (ronds) et de septembre (triangles). Il semble donc n'y avoir pas de "cohabitation" entre les communautés bactériennes en septembre (hiver) et en mars (été). 
Ensuite, on peut également relever des différences entre les différentes profondeurs par rapport à leur date. Par exemple, ...
